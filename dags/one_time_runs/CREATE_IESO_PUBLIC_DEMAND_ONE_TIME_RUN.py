"""
IESO Public Demand One-Time Historical Load

This DAG downloads ALL historical Ontario electricity demand data from IESO's
public API and performs initial bulk load into PostgreSQL.
...
@dag(
    ...
    catchup=False,
    tags=['demand', 'data-pipeline', 'ieso', 'postgres', 'one-time', 'historical', 'refactored']
)
def ieso_demand_data_pipeline():
    """
    IESO Demand Data One-Time Historical Load Pipeline

    Pipeline Steps:
    1. Get database connection string
    2. Download FULL historical demand CSV from IESO API
    ...
    def ieso_demand_data_pull() -> str:
        """
        Download FULL historical IESO demand CSV file.

        Downloads ALL available historical demand data from IESO's public API
    ...
    def transform_data(local_filename: str) -> pd.DataFrame:
        """
        Parse and transform ALL IESO demand CSV data.
