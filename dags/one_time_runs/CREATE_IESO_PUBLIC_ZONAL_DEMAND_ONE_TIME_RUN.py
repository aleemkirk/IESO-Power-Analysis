...
def ieso_zonal_demand_data_pipeline():
    ...
    _table_name = get_table_name('zonal_demand')
    _schema_name = get_schema_name('raw')
    _endpoint_url = get_ieso_url('zonal_demand')
    _filename = 'PUB_DemandZonal.csv'
    ...
    def ieso_zonal_demand_data_pull() -> str:
        ...
        """
        try:
            logger.info(f"Downloading FULL historical zonal data from {_endpoint_url}...")
    ...
    def transform_data(local_filename: str) -> pd.DataFrame:
        ...
        try:
            ...
            df['Modified_DT'] = pd.Timestamp.now(tz=_timezone).floor('S')
